...
class GiveFragment : Fragment() {
    ...
    private fun onSuccessDetecting(value: String) {
        ...
        myRef.get().addOnSuccessListener{ task ->
            val result = task
            if (result.child("quantityBook").value.toString() != "null"
                && getGlobalBookId(value).isNotEmpty() && getPrivateBookId(value).isNotEmpty()) {
                val quantityOfBook = result.child("quantityBook").value.toString().toInt()
                val nameBook = result.child("nameBook").value
                myRef.child("quantityBook").setValue(quantityOfBook - 1)
                addBookToUser(globalBookId, value, nameBook as String)
            }
        }
    }

    private fun addBookToUser(globalBookId: String, bookId: String, nameBook: String) {
        val privateBookId = getPrivateBookId(bookId)
        val myRef = Firebase.database.getReference("").child("users").child(userId!!).child("books")
        myRef.child(globalBookId).setValue(privateBookId)
    ...
    companion object {
        var TAG: String = GiveFragment::class.java.simpleName
        @JvmStatic
